...
head(data) #organización de primeros 6 datos
tail(data) #organización ultimas 6 filas

# Hacemos un primer histograma para explorar los datos
hist(data, col = "gray", main="GSE5583 - Histogram") #titulo=main

# Transformamos los datos con un logaritmo 
# ¿Qué pasa si hacemos una transformación logarítima de los datos? ¿Para qué sirve?
# Sirve para que quede bonito el gráfico
data2 = log2(data)
...
# ¿Qué es un boxplot?
# Un gráfico que nos permite representar la media (línea del medio), el q1 y q3, la dispersión de datos y los valores extremos (los más lejanos a la caja) de cada muestra diferenciando las caracteristicas de las mismas.
boxplot(data2, col=c("blue", "blue", "blue",
	"orange", "orange", "orange"),
	main="GSE5583 - boxplots", las=2). #Comando las=2 es para que ponga el nombre de las muestras en vertical
...
# de los valores de expresión. ¿Es correcta la separación? sí, vemos diferencia entre las 3 muestras wiltype y las 3 de knokout
...
wt <- data[,1:3] #Delante de la coma filas, detrás columnas
ko <- data[,4:6]
class(wt)
head(wt)
head(ko)
...
wt.mean = apply(wt, 1, mean) #media para cada gen por eso ponemos 1(fila)
...
# Calculamos la significancia estadística con un t-test. Primero deberiamos comprobar si sigue una distribución normal
...
# OJO que aquí usamos los datos SIN TRANSFORMAR. ¿Por qué? Porque en este caso es un analisis estadístico no una gráfica, no necesitamos ponerlo bonito
# ¿Cuántas valores tiene cada muestra?
pvalue = NULL 
tstat = NULL 
#Bucle para cada fila de los datos
for(i in 1 : nrow(data)) { #Para cada gen
	x = wt[i,] # gene wt número i, vector x
	y = ko[i,] # gene ko número i, vector y 
...
# Ahora vamos a marcar los que estarían sobreexpresados (rojo) y reprimidos (azul). ¿Por qué parece que están al revés? porque normalmente se ponen los sobreexpresados en azul y los reprimidos en rojo
...
# ¿Qué es cada parámetro que hemos usado dentro de la función heatmap? Los sobreexpresados en knockout estan reprimidos en wiltype y viceversa. Rojo sobreexpresados en wiltype, amarillo en knockout.
