# UNA MUESTRA T-TEST #de normal dos muestras o una sola con valor ideal
set.seed(100) #por azar se crea una semilla
x<-rnorm(50,mean=10,sd=0.5) #distribución normal random 
t.test(x,mu=10)
#p-value. mayor de 0,05 hipotesis nula, hemos pasado el corte
# UNA MUESTRA WILCOXON TEST, muestras no paramétricas
...
anova <- aov(Vigilancia~Dosis,datos_anova) #Asociación entre vigilancia y dosis
summary(anova)
#La siginificancia aumenta y aumentan los asteriscos
#Kruskal-Wallis
datos <- data.frame(
...
head(datos)
aggregate(n_huevos ~ condicion, data = datos, FUN = median) #función parecida a lapi, permite calcular la mediana de mas de una columna o fila
...
set.seed(100)
normaly_disb <- rnorm(100,mean=5,sd=1) #forzado a seguir distribucion normal
shapiro.test(normaly_disb)

set.seed(100)
not_normaly_disb <- runif(100) #distribución uniforme, no normal
...
matrix
chisq.test(matrix) #mejores resultados que con pocos valores

# FISHER
var.test(x,y)
fisher.test(matrix)
#menor numero de valores
# CORRECIÓN P VALUE
