...
install.packages("UsingR") #forma básica, utilizar España A Coruña

# Cargar los paquetes instalados
library(UsingR)
#Se descarga una vez, se puede cargar múltiples
...
# Nombres de variables: pueden incluir puntos, "_" pero NUNCA "-", se interpreta como una resta. NUNCA empiezan por números.
# Cuidado con usar nombres con palabras especiales, como nombres de funciones para nombrar variables.

num <- 10     # numérico, flecha e igual es lo mismo
...
x*y*Z #Hay distinción entre mayúsculas y minúsculas! sale error
...
rep(1,10) #repite 1 10 veces

seq(2,6) #De donde empieza a donde acaba

seq(4,20,by=4) #inicio,fin y paso

#Para navegar por ellos, se buscan sus posiciones
x[1]
x[-1] #todos los elementos menos el primero
...
bp<-read.table("bp.txt",header=T) # así leemos el archivo, muestra la primera linea de la columnaa(los nombres) como encabezado
bp
bp[1,1]
...
head(iris)    #iris solo es tabla enorme, head solo primeras filas
tail(iris)   # dataset integrado en R #final de la tabla
str(iris) #analisis variables
summary(iris)
dim(iris) #dimensiones tabla
iris$Species
...
iris[,2]
mean(iris[,2])
##### 7.- VECTORES / LISTAS AVANZADOS/LISTAS DENTRO DE LISTAS
...
ifelse(num %% #modulo 2 == 0, "Par", "Non") # Forma comprimida, más dificil de entender
...
### APPLY apply(x,margin,fun) #x datos, margin filas columnas, fun la funcion como ej media
